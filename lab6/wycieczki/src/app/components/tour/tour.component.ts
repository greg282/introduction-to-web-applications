...
export class TourComponent {
  ...
  constructor(private service:AddToCartService,private router: Router,public fire:FirebaseServiceService) {
  }
  ...
  add_click(){
    if(this.add_handle.enabled){
      this.curr_state-=1;
      this.notifyParent.emit({'val':-1,'id':this.id,'curr':this.curr_state});
    }
   this.state_service();
  ...
  openDetails(){
    this.router.navigate(['/tourlist',this.id]);
}
}
